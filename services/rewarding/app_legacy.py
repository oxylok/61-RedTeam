...
def get_args():
    ...
    parser.add_argument(
        "--reward_app_key", type=str, default=os.getenv("REWARD_APP_KEY")
    )
    ...
    parser.add_argument(
        "--reward_app_ss58", type=str, default=os.getenv("REWARD_APP_SS58")
    )
...
class RewardApp:
    def __init__(self, args):
        ...
        self.submission_scoring_logs = self.fetch_submission_scoring_logs(
            list(self.active_challenges.keys())
        )
        self.previous_submission_scoring_logs = copy.deepcopy(
            self.submission_scoring_logs
        )
        ...
        self.sync_metagraph_thread = threading.Thread(
            target=self._sync_metagraph, daemon=True
        ).start()
        self.scoring_thread = threading.Thread(
            target=self.reward_submission, daemon=True
        ).start()
        self.finalize_challenge_result_thread = threading.Thread(
            target=self.get_challenge_result_of_today, daemon=True
        ).start()

        self.app = FastAPI()
        self.app.add_api_route(
            "/get_scoring_logs", self.get_scoring_logs, methods=["GET"]
        )
    ...
    def smooth_transition_challenge(self):
        ...
        if datetime.datetime.now(datetime.timezone.utc) <= datetime.datetime(
            2025, 1, 15, 14, 0, 0, 0, datetime.timezone.utc
        ):
        ...
        for challenge in self.active_challenges.keys():
            if challenge not in self.miner_managers:
                self.miner_managers[challenge] = MinerManager(
                    ...
                    challenge_incentive_weight=self.active_challenges[challenge][
    ...
    def reward_submission(self):
        ...
        while True:
            ...
            grouped_miner_submit, self.mapping_docker_id_miner_id = (
                self.group_miner_submit_by_challenge(self.miner_submit)
            )
            ...
            if validate_scoring_hour and validate_scoring_date:
                ...
                for challenge_name in self.miner_managers:
                    if self.is_scoring_done.get(challenge_name, False):
                        scoring_logs = []
                        submission_scoring_logs = self.submission_scoring_logs[
                            challenge_name
                        ]
                        for docker_hub_id, logs in submission_scoring_logs.items():
                            for log in logs:
                                if docker_hub_id in self.mapping_docker_id_miner_id:
                                    ...
                                    scoring_logs.append(
                                        ScoringLog(
                                            uid=self.mapping_docker_id_miner_id[
                                                docker_hub_id
                                            ][
                                                "uid"
                                            ],  # TODO: Change to log["uid"] in next update
                                            ss58_address=self.mapping_docker_id_miner_id[
                                                docker_hub_id
                                            ][
                                                "ss58_address"
                                            ],  # TODO: Change to log["ss58_address"] in next update
                                            score=log["score"],
                        ...
                        bt.logging.info(
                            f"[SCORING] Scoring for challenge: {challenge_name} has been completed for {today_key}"
                        )

                        if all(
                            self.is_scoring_done.get(challenge_name, False)
                            for challenge_name in self.active_challenges.keys()
                        ):
                            bt.logging.info(
                                f"[SCORING] All tasks: Scoring completed for {today_key}"
                            )
    ...
    def run_challenges(self, docker_images_by_challenge: dict):
        ...
        for challenge_name, challenge_info in self.active_challenges.items():
            try:
                ...
                not_scored_submissions = [
                    docker_hub_id
                    for docker_hub_id in docker_images_by_challenge.get(
                        challenge_name, []
                    )
                    if docker_hub_id
                    not in self.submission_scoring_logs.get(challenge_name)
                ]
                not_scored_submissions = list(set(not_scored_submissions))
                not_scored_uid_ss58_address_pairs = [
                    (
                        self.mapping_docker_id_miner_id[docker_hub_id]["uid"],
                        self.mapping_docker_id_miner_id[docker_hub_id]["ss58_address"],
                    )
                    for docker_hub_id in not_scored_submissions
                ]
                ...
                for log in logs:
                    ...
                    if (
                        miner_docker_image
                        not in self.submission_scoring_logs[challenge_name]
                    ):
                        self.submission_scoring_logs[challenge_name][
                            miner_docker_image
                        ] = []
                    ...
                    if (
                        miner_docker_image
                        not in new_submission_scoring_logs[challenge_name]
                    ):
                        new_submission_scoring_logs[challenge_name][
                            miner_docker_image
                        ] = []
            ...
            for challenge_name, commit_data in challenges.items():
                ...
                try:
                    if "docker_hub_id" in commit_data:
                        ...
                        not commit_data.get("commit")
                        and commit_data.get("key")
                        and time.time() - commit_data["commit_timestamp"] > 24 * 60 * 60
                        and constants.is_commit_on_time(commit_data["commit_timestamp"])
                    ):
                    ...
                    if not current_submit or (
                        current_submit
                        and current_submit["commit_timestamp"]
    ...
    def _fetch_miner_submit(self):
        ...
        validator_miner_submit = (
            {}
        )  # key: validator_uid, value: dict of miner_submit for that validator, map from miner_ss58_address to miner_submit of each challenge
        for validator_uid in active_validator_uids:
            try:
                endpoint = constants.STORAGE_API.URL + "/fetch-miner-submit"
                data = {
                ...
                if response.status_code == 200:
                    ...
                    for miner_ss58_address, challenges in data["miner_submit"].items():
                        if miner_ss58_address in self.metagraph.hotkeys:
                            ...
                            continue
                        for challenge_name, commit_data in challenges.items():
                            validator_miner_submit.setdefault(
                                validator_uid, {}
                            ).setdefault(miner_ss58_address, {})[challenge_name] = {
                                "commit_timestamp": commit_data["commit_timestamp"],
                    ...
                    print(
                        f"[SUCCESS] Fetched miner submit data from storage for validator {validator_uid}."
                    )
                else:
                    print(
                        f"[ERROR] Failed to fetch miner submit data: {response.status_code} - {response.text}"
                    )
            except Exception as e:
                print(
                    f"[ERROR] Error fetching miner submit data from storage for validator {validator_uid}: {e}"
                )
        ...
        for (
            validator_uid,
            this_validator_miner_submit,
        ) in validator_miner_submit.items():
            for (
                miner_ss58_address,
                miner_submissions,
            ) in this_validator_miner_submit.items():
                if miner_ss58_address not in miner_submit:
                    # Update if first met this miner_ss58_address
                    miner_submit[miner_ss58_address] = this_validator_miner_submit[
                        miner_ss58_address
                    ]
                else:
                    for challenge_name, commit_data in miner_submissions.items():
                        if challenge_name not in miner_submit[miner_ss58_address]:
                            miner_submit[miner_ss58_address][
                                challenge_name
                            ] = commit_data
                        else:
                            current_miner_submit_for_challenge = miner_submit[
                                miner_ss58_address
                            ][challenge_name]
                            if (
                                commit_data["encrypted_commit"]
                                == current_miner_submit_for_challenge[
                                    "encrypted_commit"
                                ]
                            ):
                                # If encrypted commit is the same, we update to older commit timestamp and add unknown commit and key field if possible
                                miner_submit[miner_ss58_address][challenge_name][
                                    "commit_timestamp"
                                ] = commit_data["commit_timestamp"]
    ...
    def fetch_submission_scoring_logs(self, challenge_names: list):
        endpoint = constants.STORAGE_API.URL + "/fetch-centralized-score"

        submission_scoring_logs = {}
        try:
            response = requests.post(
                endpoint, json={"challenge_names": challenge_names}
            )
            if response.status_code == 200:
                print(
                    "[SUCCESS] Submission scoring logs successfully fetched from storage."
                )
                ...
                print(
                    f"[ERROR] Failed to fetch submission scoring logs from storage: {response.status_code} - {response.text}"
                )
    ...
    def save_submission_scoring_logs(self, new_submission_scoring_logs: dict):
        endpoint = constants.STORAGE_API.URL + "/upload-centralized-score"
        try:
            ...
            #     return False
            response = requests.post(
                endpoint, json={"data": new_submission_scoring_logs}
            )
            if response.status_code == 200:
                print(
                    "[SUCCESS] Submission scoring logs successfully saved to storage."
                )
                self.previous_submission_scoring_logs = copy.deepcopy(
                    self.submission_scoring_logs
                )
                ...
                print(
                    f"[ERROR] Failed to save submission scoring logs to storage: {response.status_code} - {response.text}"
                )
        except Exception as e:
    ...
    def _init_challenge_records_from_subnet(self, is_today_scored: bool = False):
        try:
            endpoint = constants.STORAGE_API.URL + "/fetch-challenge-records"
            data = {
            ...
            if response.status_code == 200:
                ...
                for challenge_name, challenge_record in data.items():
                    if challenge_name in self.miner_managers:
                        self.miner_managers[challenge_name].challenge_records = {
                            date: ChallengeRecord(**record)
                            for date, record in challenge_record.items()
                        }
                bt.logging.success(
                    "[INIT] Challenge records data successfully initialized from storage."
                )
            else:
                bt.logging.error(
                    f"[INIT] Failed to fetch challenge records data: {response.status_code} - {response.text}"
                )
        except Exception as e:
            bt.logging.error(
                f"[INIT] Error initializing challenge records data from storage: {e}"
            )
    ...
    def _store_challenge_records(self):
        ...
        for challenge_name, miner_manager in self.miner_managers.items():
            challenge_records[challenge_name] = {
                date: record.__dict__
                for date, record in miner_manager.challenge_records.items()
            }
        data = {
            "validator_ss58_address": self.REWARD_APP_SERVER_SS58_ADDRESS,
