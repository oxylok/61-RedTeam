...
def test_eslint() -> JSONResponse:
    try:
        response = requests.post(
            ...
            timeout=60,
            json={
                "js_content": _load_fingerprinter_js(),
            },
        )
...
def _load_fingerprinter_js() -> str:
    try:
        ...
        with open(_fingerprinter_js_path, "r") as _fingerprinter_js_file:
            return _fingerprinter_js_file.read()
    except Exception as e:
        logger.error(f"Failed to load fingerprinter.js: {e}")
        raise HTTPException(status_code=500, detail="Failed to load fingerprinter.js")
...
def solve(miner_input: MinerInput = Body(...)) -> MinerOutput:
    ...
    try:
        ...
        # _requirements_txt_path = str(_fingerprinter_dir / "requirements.txt")
        ...
        # _system_deps_path = str(_fingerprinter_dir / "system_deps.txt")
