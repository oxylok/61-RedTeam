...
class PathsConfig(BaseConfig):
    configs_dir: constr(strip_whitespace=True) = Field(..., min_length=2, max_length=1024)  # type: ignore
    ...
    @classmethod
    def _check_configs_dir(cls, val: str) -> str:
        _configs_dir_env = f"{ENV_PREFIX_API}CONFIGS_DIR"
        if _configs_dir_env in os.environ:
            val = os.getenv(_configs_dir_env)
...
class FrozenPathsConfig(PathsConfig):
    ...
    def _check_all(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        for _key, _val in values.items():
            ...
            if isinstance(_val, str) and ("{tmp_dir}" in _val):
