...
class DFPController(Controller):
    ...
    def __init__(
        ...
        for docker_hub_id in self.baseline_reference_comparison_docker_hub_ids:
            ...
            if docker_hub_id in DFPController._baseline_reference_cache:
                cached_commit = DFPController._baseline_reference_cache[docker_hub_id]
                # Verify it has scoring logs (i.e., has been successfully scored)
                if cached_commit.scoring_logs:
                    bt.logging.info(
                        f"[CONTROLLER - DFPController] Reference commit {docker_hub_id} has already been scored, skipping"
                    )
                    continue
    ...
    def start_challenge(self):
        ...
        bt.logging.debug(
            f"[CONTROLLER - DFPController] Generated {len(challenge_inputs)} challenge inputs"
        )

        bt.logging.info(
            f"[CONTROLLER - DFPController] Starting baseline reference scoring for {len(self.baseline_reference_comparison_commits_to_score)} references"
        )

        for reference_commit in self.baseline_reference_comparison_commits_to_score:
            try:
                bt.logging.info(
                    f"[CONTROLLER - DFPController] Scoring baseline reference: {reference_commit.docker_hub_id}"
                )
                ...
                bt.logging.info(
                    f"[CONTROLLER - DFPController] Baseline reference scoring logs: {len(reference_commit.scoring_logs)}"
                )
                DFPController._baseline_reference_cache[
        ...
        bt.logging.debug(
            f"[CONTROLLER - DFPController] Starting miner scoring for {len(self.miner_commits)} miners"
        )
        ...
        bt.logging.debug(
            f"[CONTROLLER - DFPController] Challenge completed, cleaning up challenge container"
        )
    ...
    def _get_all_reference_commits(self):
        return self.reference_comparison_commits + list(
            DFPController._baseline_reference_cache.values()
        )
